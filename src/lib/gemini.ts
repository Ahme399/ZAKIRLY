...
export const getStudyAdvice = async () => {
  try {
    const response = await ai.models.generateContent({
      model: chatModel,
      contents: "أعطني نصيحة دراسية واحدة سريعة وملهمة للطالب المصري بالعربي بالعامية المصرية بأسلوب مشجع جداً وفكاهي قليلاً وتكون مختلفة في كل مرة.",
      config: {
        systemInstruction: "أنت مساعد ذكي اسمك 'ذاكرلي'، خبير في المناهج المصرية والتحفيز الدراسي. تتحدث بالعامية المصرية الودودة. قدم نصيحة واحدة فقط قصيرة جداً.",
      }
    });
...
export const askAI = async (message: string, history: { role: string; content: string }[] = [], stage: string = "غير محدد", language: string = "arabic") => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: `أنت مساعد دراسي ذكي جداً وعالمي اسمك 'ذاكرلي'. أنت بروفيسور خبير في المناهج المصرية (عربي وإنجليزي وعلوم ورياضيات إلخ) وفي نفس الوقت ملم بكل العلوم العامة. 
        أسلوبك: بروفيشنال، متطور، مشجع، وواضح جداً. أنت تتحدث بالعامية المصرية الودودة لكن بدقة علمية عالية.
        ...
        قواعدك:
        1. جاوب على أي سؤال يطرحه الطالب سواء دراسي أو عام، لكن ابدأ بربطه بذكاء بحياته الدراسية إذا أمكن.
        2. إذا سألك الطالب عن مسألة، اشرحها له ببساطة وفصلها حسب منهجه المصري الحالي.
        3. إذا طلب تدريباً، قدم له أسئلة ذكية تتوافق مع نظام الامتحانات المصري الجديد (Open Book / Multiple Choice).
        4. كن متطوراً في تفكيرك، استخدم أمثلة من الواقع التكنولوجي الحديث.
        5. إذا كان الطالب في نظام اللغات، استخدم المصطلحات الإنجليزية (Terms) بجانب الشرح العربي للـ Math و Science.`,
      }
    });
