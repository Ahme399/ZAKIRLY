...
import { Plus, Trash2, Search, Edit3, Save, Share2, NotebookPen, Clock, ArrowRight } from 'lucide-react';
...
export default function NoteEditor() {
  ...
  return (
    <div className="h-[calc(100vh-12rem)] flex flex-col gap-8 pb-4">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 shrink-0">
        ...
        <button 
          onClick={addNote}
          className="bg-[#00D1FF] text-black px-8 py-4 rounded-2xl font-black shadow-xl shadow-cyan-500/10 hover:scale-105 transition-all flex items-center gap-3 w-full md:w-auto justify-center"
        >
          <Plus size={24} />
          إضافة نوت جديد
        </button>
      ...
      <div className="flex flex-col lg:flex-row gap-8 flex-1 min-h-0">
        {/* Sidebar List */}
        <aside className={`lg:w-1/3 flex flex-col gap-6 overflow-hidden ${editingId ? 'hidden lg:flex' : 'flex'}`}>
          <div className="relative group shrink-0">
            ...
            <input 
              ...
              className="w-full bg-[#0C0D11] border border-gray-800 rounded-2xl px-14 py-4 focus:outline-none focus:border-cyan-500/30 font-bold text-gray-200 shadow-xl transition-all text-right"
            />
          </div>
            ...
            <AnimatePresence initial={false}>
              {filteredNotes.map((note) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-3 relative z-10 rtl">
                    <h3 className={`font-black text-lg truncate pr-2 ${editingId === note.id ? 'text-[#00D1FF]' : 'text-gray-200'}`}>
                      {note.title || 'بدون عنوان'}
                    </h3>
                    <div className={`w-3 h-3 rounded-full shadow-lg shrink-0 ${note.color.replace('/40', '')}`}></div>
                  </div>
                  <p className="text-gray-500 text-xs font-bold line-clamp-2 h-10 leading-relaxed overflow-hidden relative z-10 text-right">
        ...
        {/* Editor Area */}
        <main className={`lg:flex-1 bg-[#0C0D11] rounded-[2.5rem] border border-gray-800 shadow-2xl overflow-hidden flex flex-col relative ${!editingId ? 'hidden lg:flex' : 'flex h-full lg:h-auto'}`}>
          <div className="absolute top-0 right-0 w-64 h-64 bg-cyan-500/5 blur-3xl rounded-full"></div>
          {activeNote ? (
            <div className={`flex-1 flex flex-col p-6 md:p-12 space-y-8 relative z-10 ${activeNote.color}`}>
              <div className="flex items-center justify-between border-b border-gray-800 pb-6 gap-4">
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  <button 
                    onClick={() => setEditingId(null)}
                    className="lg:hidden p-2 text-gray-400 hover:text-white transition-colors border border-gray-800 rounded-xl shrink-0"
                  >
                    <ArrowRight size={20} />
                  </button>
                  <input 
                    ...
                    className="text-2xl md:text-3xl font-black text-white bg-transparent focus:outline-none flex-1 placeholder:text-gray-700 min-w-0 text-right"
                ...
                <div className="flex gap-2 md:gap-3 shrink-0">
                   <button 
                     ...
                     className="p-2 md:p-3 text-gray-600 hover:text-rose-500 hover:bg-rose-500/10 rounded-2xl transition-all"
                     title="حذف المذكرة"
                   >
                     <Trash2 size={24} />
                   </button>
                   <button 
                     onClick={() => setEditingId(null)} 
                     className="p-2 md:p-3 text-[#00D1FF] hover:bg-cyan-500/10 rounded-2xl transition-all flex items-center gap-2"
                     title="إنهاء التحرير"
                   >
                     <Save size={24} />
                     <span className="hidden md:inline font-bold text-xs uppercase tracking-widest">حفظ</span>
                   </button>
                </div>
              </div>
              <textarea 
                ...
                className="flex-1 w-full text-gray-300 leading-relaxed focus:outline-none resize-none font-bold text-lg md:text-xl bg-transparent placeholder:text-gray-800 no-scrollbar text-right"
                placeholder="الغوص في التفاصيل... اكتب كل اللي في بالك هنا..."
              />
              <div className="flex flex-col md:flex-row md:items-center justify-between pt-8 border-t border-gray-800 gap-6 shrink-0 mt-auto">
                 <div className="flex items-center gap-3 text-gray-500 text-[10px] font-black uppercase tracking-widest order-2 md:order-1">
                   <Clock size={14} />
                   <span>آخر تعديل: {new Date(activeNote.updatedAt).toLocaleString()}</span>
                 </div>
                 <div className="flex items-center gap-6 order-1 md:order-2 justify-between md:justify-end">
